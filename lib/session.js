...
import { expandPath } from './keychain.js';
...
export function findSessionAcrossProfiles(accounts, sessionId) {
  ...
  for (const account of accounts) {
    const expanded = expandPath(account.configDir);
...
export function migrateSessionByHash(fromConfigDir, toConfigDir, cwdHash, sessionId) {
  validateSessionId(sessionId);
  try {
    const expandFrom = expandPath(fromConfigDir);
    const expandTo = expandPath(toConfigDir);
