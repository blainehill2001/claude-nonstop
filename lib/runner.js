...
const { drainMessageQueue, findSessionIdByTmux } = require('../remote/channel-manager.cjs');
...
function spawnHookNotify(type, data) {
  ...
  try {
    ...
    // Fire-and-forget — child may have exited before stdin write
  }
  child.unref();
}
...
/**
 * Set the sleeping flag on all active channel-map entries for a tmux session.
 */
function setSleepingFlag(tmuxSessionName, sleeping) {
  const channelMapPath = path.join(CONFIG_DIR, 'data', 'channel-map.json');
  try {
    if (!fs.existsSync(channelMapPath)) return;
    const raw = fs.readFileSync(channelMapPath, 'utf8');
    if (!raw.trim()) return;
...
/**
 * Replay queued messages by sending them to tmux.
 */
async function replayQueuedMessages(tmuxSession) {
  const messages = drainMessageQueue();
  for (const msg of messages) {
    if (msg.tmuxSession === tmuxSession && msg.text) {
      try {
        await new Promise((resolve, reject) => {
          execFile('tmux', ['send-keys', '-t', tmuxSession, '-l', msg.text], { timeout: 5000 }, (err) => {
            if (err) reject(err); else resolve();
          });
        });
        await new Promise(resolve => setTimeout(resolve, 300));
        await new Promise((resolve, reject) => {
          execFile('tmux', ['send-keys', '-t', tmuxSession, 'Enter'], { timeout: 5000 }, (err) => {
            if (err) reject(err); else resolve();
          });
        });
  ...
  while (swapCount <= maxSwaps) {
    ...
    if (best && effectiveUtilization(best.account.usage) >= EXHAUSTION_THRESHOLD) {
      ...
      if (sleepMs > 0) {
        ...
        // Flush any remaining output before sleep
        if (remoteAccess && sessionId) {
        ...
        const tmuxName = remoteAccess ? getCurrentTmuxSession() : null;

        if (remoteAccess) {
          // Mark session as sleeping so webhook queues messages
          if (tmuxName) setSleepingFlag(tmuxName, true);
        ...
        if (remoteAccess && tmuxName) {
          setSleepingFlag(tmuxName, false);
        }
        ...
        if (remoteAccess) {
          ...
          // Replay queued messages from sleep period
          if (tmuxName) {
            await replayQueuedMessages(tmuxName);
          }
        }
...
function runOnce(claudeArgs, account, existingSessionId, options = {}) {
  return new Promise((resolve) => {
    ...
    child.onExit(({ exitCode }) => {
      ...
      // Final output flush
      if (options.remoteAccess && sessionId) {
        const bufPath = outputBufferPath(sessionId);
        const sigPath = outputSignalPath(sessionId);
        const content = readOutputBuffer(bufPath);
        if (content) {
